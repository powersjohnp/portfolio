...
cat stim_files_list.txt | while read line; do
	...
	if [ ${order} == 1 ]; then
		if [ $(( ${run} % 2)) == 0 ]; then
			...
			# write stim file for self
			echo "${event_time} ${event_duration} 1" > ${data_dir}/${id}/self_run${run}.txt
			
			# write stim file for self_look_neu 
			cat ${data_dir}/ROSEF_timing_${id}_${order}_${run}.txt | awk 'NR>1 {if ($3==2) {print $6, 7, 1}}' > ${data_dir}/${id}/self_look_neu_run${run}.txt
	
			# write stim file for self_look_neg 
			cat ${data_dir}/ROSEF_timing_${id}_${order}_${run}.txt | awk 'NR>1 {if ($3==1 && $4==2) {print $6, 7, 1}}' > ${data_dir}/${id}/self_look_neg_run${run}.txt
	
			# write stim file for self_change_neg
			cat ${data_dir}/ROSEF_timing_${id}_${order}_${run}.txt | awk 'NR>1 {if ($4==1) {print $6, 7, 1}}' > ${data_dir}/${id}/self_change_neg_run${run}.txt
			
			# write stim file for other
			touch ${data_dir}/${id}/other_run${run}.txt
			
			# write stim file for other_look_neu 
			touch ${data_dir}/${id}/other_look_neu_run${run}.txt
	
			# write stim file for other_look_neg 
			touch ${data_dir}/${id}/other_look_neg_run${run}.txt
	
			# write stim file for other_change_neg
			touch ${data_dir}/${id}/other_change_neg_run${run}.txt
	
		else
		
			# write stim file for other
			echo "${event_time} ${event_duration} 1" > ${data_dir}/${id}/other_run${run}.txt
			
			# write stim file for other_look_neu 
			cat ${data_dir}/ROSEF_timing_${id}_${order}_${run}.txt | awk 'NR>1 {if ($3==2) {print $6, 7, 1}}' > ${data_dir}/${id}/other_look_neu_run${run}.txt
	
			# write stim file for other_look_neg 
			cat ${data_dir}/ROSEF_timing_${id}_${order}_${run}.txt | awk 'NR>1 {if ($3==1 && $4==2) {print $6, 7, 1}}' > ${data_dir}/${id}/other_look_neg_run${run}.txt
	
			# write stim file for other_change_neg
			cat ${data_dir}/ROSEF_timing_${id}_${order}_${run}.txt | awk 'NR>1 {if ($4==1) {print $6, 7, 1}}' > ${data_dir}/${id}/other_change_neg_run${run}.txt
			
			# write stim file for self
			touch ${data_dir}/${id}/self_run${run}.txt
			
			# write stim file for self_look_neu 
			touch ${data_dir}/${id}/self_look_neu_run${run}.txt
	
			# write stim file for self_look_neg 
			touch ${data_dir}/${id}/self_look_neg_run${run}.txt
	
			# write stim file for self_change_neg
			touch ${data_dir}/${id}/self_change_neg_run${run}.txt
